...
import { Search, Plus, Pencil, Trash2, Mail, Hash, X } from 'lucide-react'
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import toast from 'react-hot-toast'
import Layout from '../components/layout/Layout'
import Pagination from '../components/ui/Pagination'
import { PageLoader, EmptyState } from '../components/ui/Skeleton'
import { etudiantService } from '../services/crudService'

const EMPTY = { nom: '', prenom: '', email: '', sexe: 'M', date_naissance: '' }
...
export default function Etudiants() {
  ...
  const { data, isLoading: loading } = useQuery({
    queryKey: ['etudiants', page, debouncedSearch, filtreSexe],
    queryFn:  () => etudiantService.getAll(page, debouncedSearch, filtreSexe),
  ...
  const createMutation = useMutation({
    mutationFn: (d) => etudiantService.create(d),
    onSuccess: (created) => {
      toast.success(`Étudiant ajouté — Matricule : ${created.matricule}`)
      fermer(); invalidate()
    },
    onError: (err) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, d }) => etudiantService.update(id, d),
    onSuccess: () => { toast.success('Étudiant modifié'); fermer(); invalidate() },
    onError: (err) => {
      const data = err.response?.data
      if (data?.errors) Object.values(data.errors).forEach((msgs) => msgs.forEach((m) => toast.error(m)))
      else toast.error(data?.message ?? 'Une erreur est survenue')
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => etudiantService.remove(id),
    onSuccess: () => { toast.success('Étudiant archivé'); setConfirmDel(null); invalidate() },
  ...
  const handleFiltreSexe = (val) => { setFiltreSexe(val); setPage(1); setRecherche(''); setDebouncedSearch('') }

  const ouvrirCreation = () => { setEditing(null); setForm(EMPTY); setErrors({}); setShowModal(true) }
  const ouvrirEdition  = (e) => {
    setEditing(e)
    setForm({ nom: e.nom, prenom: e.prenom, email: e.email, sexe: e.sexe, date_naissance: e.date_naissance ?? '' })
  ...
  const valider = () => {
    ...
    if (!form.date_naissance) e.date_naissance = 'Requis'
      ...
      const age = (new Date() - new Date(form.date_naissance)) / (1000 * 60 * 60 * 24 * 365)
      if (age < 16) e.date_naissance = "L'étudiant doit avoir au moins 16 ans"
    }
    setErrors(e)
  ...
  return (
    <Layout>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-medium text-gray-900">Étudiants</h1>
          <p className="text-xs text-gray-400 mt-0.5">{meta?.total ?? 0} étudiant{(meta?.total ?? 0) > 1 ? 's' : ''} enregistré{(meta?.total ?? 0) > 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        <select value={filtreSexe} onChange={(e) => handleFiltreSexe(e.target.value)}
        ...
        {(recherche || filtreSexe) && (
          <button onClick={() => { setRecherche(''); setDebouncedSearch(''); setFiltreSexe(''); setPage(1) }}
      ...
      {loading ? (
        ...
        <EmptyState title="Aucun étudiant trouvé"
          description={debouncedSearch ? `Aucun résultat pour "${debouncedSearch}"` : 'Ajoutez votre premier étudiant'}
          actionLabel={!debouncedSearch ? '+ Nouvel étudiant' : undefined}
              ...
              <motion.div key={e.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-11 h-11 rounded-lg flex items-center justify-center text-sm font-medium flex-shrink-0 ${e.sexe === 'F' ? 'bg-pink-100 text-pink-700' : 'bg-trt-100 text-trt-700'}`}>
                      {initiales(e)}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-900">{e.prenom} {e.nom}</p>
                      <span className={`text-[11px] font-medium px-1.5 py-0.5 rounded ${e.sexe === 'F' ? 'bg-pink-50 text-pink-600' : 'bg-blue-50 text-blue-600'}`}>
                        {e.sexe === 'F' ? 'Féminin' : 'Masculin'}
                      </span>
                    </div>
                  </div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => ouvrirEdition(e)} className="w-7 h-7 rounded-md hover:bg-trt-100 flex items-center justify-center transition-colors">
                      <Pencil size={12} className="text-gray-400" />
                    </button>
                    <button onClick={() => setConfirmDel(e)} className="w-7 h-7 rounded-md hover:bg-red-50 flex items-center justify-center transition-colors">
                ...
                <div className="space-y-2 border-t border-gray-100 pt-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400 font-mono">{e.matricule}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Mail size={12} className="text-gray-300 flex-shrink-0" />
                    <span className="text-xs text-gray-500 truncate">{e.email}</span>
                  </div>
                  {fmt(e.date_naissance) && <p className="text-xs text-gray-400 pl-5">Né le {fmt(e.date_naissance)}</p>}
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <h2 className="text-sm font-semibold text-gray-900">{editing ? "Modifier l'étudiant" : 'Nouvel étudiant'}</h2>
                <button onClick={fermer} className="text-gray-400 hover:text-gray-600 transition-colors"><X size={16} /></button>
              </div>
              <div className="px-6 py-5">
                {!editing && (
                  <div className="bg-trt-100 rounded-md px-3 py-2.5 mb-4">
                    <p className="text-xs text-trt-700">Matricule généré automatiquement : <span className="font-mono font-medium">TRT-{new Date().getFullYear()}-{(form.prenom?.[0] ?? 'P').toUpperCase()}{(form.nom?.[0] ?? 'N').toUpperCase()}-XXXX</span></p>
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input type="date" value={form.date_naissance}
                        onChange={(e) => { setForm({ ...form, date_naissance: e.target.value }); setErrors({ ...errors, date_naissance: '' }) }}
                        max={new Date(new Date().setFullYear(new Date().getFullYear() - 16)).toISOString().split('T')[0]}
                        className={`w-full text-xs border rounded-md px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-trt-500 focus:border-trt-500 ${errors.date_naissance ? 'border-red-300' : 'border-gray-200'}`} />
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="px-6 pt-6 pb-5">
                ...
                <h2 className="text-sm font-semibold text-gray-900 mb-1">Archiver l'étudiant</h2>
                <p className="text-xs text-gray-500 mb-5">
                  Voulez-vous archiver <span className="font-medium text-gray-800">{confirmDel.prenom} {confirmDel.nom}</span> ({confirmDel.matricule}) ? L'historique sera préservé.
                </p>
